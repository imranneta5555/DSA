...
def setup_document() -> Document:
    ...
    core.title = f"{COVER['unit_title']}: {COVER['assignment_title']}"
...
def cover_sheet(doc, word_count: int) -> None:
    """The standard Learn Key front page, following the official form's layout."""
    head = doc.add_table(rows=1, cols=2)
    head.autofit = False
    cells = head.rows[0].cells
    for cell in cells:
        cell_borders(cell, edges=())
    set_column_widths(head, [5.3, 10.6])

    logo = cells[0].paragraphs[0]
    logo.paragraph_format.line_spacing = 1.0
    logo.add_run().add_picture(str(LOGOS / "learnkey.png"), height=Cm(1.7))

    title = cells[1].paragraphs[0]
    title.paragraph_format.space_before = Pt(4)
    title.paragraph_format.space_after = Pt(4)
    title.paragraph_format.line_spacing = 1.0
    run = title.add_run("Assignment Cover Sheet")
    set_run_font(run, BODY_FONT, 16)
    run.bold = True

    accreditation = cells[1].add_table(rows=0, cols=2)
    accreditation.alignment = WD_TABLE_ALIGNMENT.RIGHT
    accreditation.autofit = False
    for filename, height in (("othm.png", 1.05), ("mfhea.png", 1.0)):
        ...
        picture.add_run().add_picture(str(LOGOS / filename), height=Cm(height))
        cell_borders(left)
        cell_borders(right)
    set_column_widths(accreditation, [5.4, 1.6])
    closing = cells[1].paragraphs[-1]  # Word requires a paragraph after a nested table
    ...
    instruction.paragraph_format.space_after = Pt(6)
    run = instruction.add_run("This cover sheet must be completed and added to the front of every assignment")
    set_run_font(run, BODY_FONT, 12)
    run.bold = True

    def form_cell(cell, text, bold=False, italic=False, shaded=False):
        paragraph = cell.paragraphs[0]
        paragraph.style = doc.styles["Table Text"]
        piece = paragraph.add_run(text)
        set_run_font(piece, BODY_FONT, 12)
        piece.bold, piece.italic = bold, italic
        cell_borders(cell)
        if shaded:
    ...
    for label, value, bold_value in fields:
        row = form.add_row()
        form_cell(row.cells[0], label, bold=True, italic=True, shaded=True)
        form_cell(row.cells[1], value, bold=bold_value)
    set_column_widths(form, [5.6, 10.3])

    declaration_table = doc.add_table(rows=1, cols=1)
    set_column_widths(declaration_table, [15.9])
    declaration = declaration_table.rows[0].cells[0]
    heading = declaration.paragraphs[0]
    heading.style = doc.styles["Table Text"]
    heading.paragraph_format.space_after = Pt(4)
    run = heading.add_run("Declaration of authenticity:")
    set_run_font(run, BODY_FONT, 12)
    run.bold = True
    statements = [
    ...
    for number, statement in enumerate(statements, start=1):
        paragraph = declaration.add_paragraph(style="Table Text")
        ...
        run = paragraph.add_run(f"{number}.\t{statement}")
        set_run_font(run, BODY_FONT, 12)
        run.bold = True
    ...
    set_column_widths(signature_table, [7.95, 7.95])
    signatures = signature_table.rows[0]
    for index, label in enumerate(("Learner signature", "Tutor signature")):
        cell = signatures.cells[index]
        top = cell.paragraphs[0]
        top.style = doc.styles["Table Text"]
        ...
        for text, is_rule in (("_________________", True), (label, False), ("Date:", False)):
            paragraph = cell.add_paragraph(style="Table Text")
            paragraph.alignment = WD_ALIGN_PARAGRAPH.CENTER
            paragraph.paragraph_format.space_after = Pt(1)
            run = paragraph.add_run(text)
            set_run_font(run, BODY_FONT, 12)
            run.bold = run.italic = not is_rule
        cell_borders(cell)

    note_table = doc.add_table(rows=1, cols=1)
    set_column_widths(note_table, [15.9])
    note = note_table.rows[0].cells[0]
    first = note.paragraphs[0]
    first.style = doc.styles["Table Text"]
    set_run_font(first.add_run("Note:"), BODY_FONT, 12)
    second = note.add_paragraph(style="Table Text")
    set_run_font(second.add_run("Assignments must be submitted in typed PDF format only; handwritten "
                                "assignments are not accepted."), BODY_FONT, 12)
    cell_borders(note)
